...
// eslint-disable-next-line camelcase
externals['@opentelemetry/exporter-trace-otlp-grpc'] =
  'next/dist/compiled/@opentelemetry/exporter-trace-otlp-grpc'
export async function ncc_opentelemetry_exporter_trace_otlp_grpc(task, opts) {
  await task
    .source(
      opts.src ||
        relative(
    ...
    .ncc({ packageName: '@opentelemetry/exporter-trace-otlp-grpc', externals })
    .target('compiled/@opentelemetry/exporter-trace-otlp-grpc')
}

// eslint-disable-next-line camelcase
externals['@opentelemetry/otlp-grpc-exporter-base'] =
  'next/dist/compiled/@opentelemetry/otlp-grpc-exporter-base'
export async function ncc_opentelemetry_otlp_grpc_exporter_base(task, opts) {
  await task
    .source(
      opts.src ||
        relative(
          __dirname,
          require.resolve('@opentelemetry/otlp-grpc-exporter-base')
        )
    )
    .ncc({ packageName: '@opentelemetry/otlp-grpc-exporter-base', externals })
    .target('compiled/@opentelemetry/otlp-grpc-exporter-base')
}
  ...
  await task
    ...
    .parallel(
      [
        ...
        'ncc_opentelemetry_exporter_trace_otlp_grpc',
        'ncc_opentelemetry_otlp_grpc_exporter_base',
